...
- **The cobble-curtain fix**: build floors now ignore tree canopies (MOTION_BLOCKING_NO_LEAVES) - settlements in forests floor on real soil instead of standing on leaves with supports down to it - and the automatic support sweep stamps a **short blended footing** (three blocks of stone, then dirt the grass takes over, ten at most) instead of full-depth stone curtains down every cliff; water still stops the fill so piers and stilts keep their legs

- **The floating-structures kill, second strike - and the invisible-item fix**: ground probes now **pull every chunk to FULL before trusting its heightmap and then verify the reported surface block is actually there** (a lying heightmap gets walked down to honest terrain) - garbage probes were the root cause of structures at sky height; the automatic foundation sweep now stamps **uncapped support all the way to the ground** (water still stops it) and an argument-order bug that silently disarmed the first sweep is fixed; expansions force-load their ground too; and the **seven held items (guns, swords, halberd, hoe, warhorn) render again** - their model parent chain had lost its geometry (parented to transforms with no shape), so they showed as empty slots; hand sizes now use the **exact vanilla block-item and handheld display numbers**, so blocks sit at true Minecraft scale in hand instead of filling the screen

- **The no-floating overhaul and smarter settlers**: world settlements no longer build inside the chunk-load callback - the site is found, claimed and **queued**, then built on the next server tick with every neighbor chunk loaded (the same proven path as `/rivalrealms rebuild`), which kills the root cause of **half a structure floating**; on top of that, a global **no-float guarantee** now runs after every build, expansion and repair - any placed block left hanging over open air gets a stone plinth stamped down to honest ground automatically (water stays the business of piers and stilts); sites marked but lost to a restart **self-heal** on the next chunk visit; and the settler AI got real upgrades: **ranged shooters keep their distance** (backing away from close threats, closing on far ones), **guards answer their neighbours' fights** and dogpile raiders, **civilians drift home when night falls**, and fleeing settlers **run toward the town guards** instead of blindly away

- **The finish pass: textures, hands, animation and the living road**: every custom block and item texture reworked (richer color, deeper edge shading, crisp dark outlines on items); **swords, guns, the halberd, the hoe and the warhorn are true 3D-anchored models in hand** - player or NPC, first or third person - with standard display transforms so nothing floats sideways anymore, and all 19 block items carry proper hand/display poses; settlers **swing their arm, kick up powder smoke and fire visible tracers** on every shot and your own revolver swings with the trigger; the **/rivalrealms chronicle opens with the realm's date - YEAR N, DAY M** - and every entry is dated the same way; once a day a **merchant caravan with two hired swords and a pack mule walks the road between real settlements** (announced in the chronicle, guards fight for it, the guild records its arrival), and settlers now have quiet **hobbies** - strolling to the green to play, music notes and all

- **Every structure rebuilt from reference builds** - *complete*: keeps now have the **great hall** (red carpet aisle, banquet benches, wall banners); palaces a gold-inlaid throne approach; western saloons a **false front and balcony**; warehouses a chain **hoist** with cargo rows; the citadel bridge is a true **arch with lanterned parapets**; the outpost palisade wears **sharpened stakes** and a flanked gate; farms and gardens use **dry footpaths instead of open water** (the water leak is closed for good); wells got their **draw-roof and windlass chain**, hangars their **timber arch vault**, quay cranes their A-frame and hanging load, storage yards a shaded canopy, lamp posts a stone footing, hay stacks a thatched cap; plus the pirate coves, drum gate, cove piers, shrines, galleons, watchtowers, mine adits, lumber camps, stables, lockups, lighthouses, smithies, barbicans, cranes, hangars and pub fronts from earlier waves
